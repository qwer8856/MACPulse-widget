...
当前版本：**3.4.2**。安装后的应用和小组件名称均为 **“系统状态”**。

## 下载安装

**[下载安装包：MacPulse Widget 3.4.2（M 系列 Mac）](https://github.com/qwer8856/MACPulse-widget/releases/download/v3.4.2/MACPulse-Widget-v3.4.2-macOS-arm64.zip)**

[版本发布页与校验文件](https://github.com/qwer8856/MACPulse-widget/releases/tag/v3.4.2)
...
| 磁盘 | 打开后自动扫描当前目录，按已分配容量降序查看子项目及占比，可选择其他目录、进入子目录、取消扫描或在 Finder 中定位 |
| 功率 | 供电功率、瓦数刻度，以及按 CPU 能耗估算降序排列的进程 |
| 电池 | 电池电量、供电与充电状态、系统可提供的预计时间和健康状态，不包含功率或进程能耗列表 |
...
首次打开磁盘详情会在后台自动扫描用户主目录，之后扫描当前选定的目录。完成结果缓存 60 秒，缓存过期后再次打开详情会自动重新扫描，也可点击刷新立即扫描。短暂经过磁盘菜单不会立刻扫描；扫描期间可查看已统计的结果。收起菜单或关闭窗口后，只要应用仍在运行，已经开始的扫描会继续；退出应用会取消扫描。目录扫描不按每秒执行。

**自动扫描默认跳过隐私目录**，包括桌面、文稿、下载、图片、音乐、影片、用户资料库、废纸篓和照片资料库等，同时跳过其他用户目录及外接卷。扫描前先检查范围，避免因自动遍历这些位置触发访问请求。被跳过的项目不计入目录合计，底部显示跳过数量；磁盘总容量与已用比例不受影响。需要扫描受保护目录时，通过目录选择器明确选中该目录，macOS 仍可能要求授权。选择整个主目录不会自动包含所有受保护子目录。
...
| 磁盘 | 总容量、已用/可用容量，以及自动扫描的目录容量与占比 |
| 功率 | 供电功率、瓦数刻度和进程 CPU 能耗排名 |
| 电池 | 电量、充电/供电状态、系统提供的预计时间与健康信息，使用独立紧凑面板 |

子菜单中的进程表支持搜索、排序和滚动。需要退出进程时，点击底部“资源监视”会直接打开对应页面，再选择进程并确认退出。磁盘子菜单可扫描当前目录、进入子目录、取消或定位文件；选择其他目录时会打开系统目录选择器，选定后开始扫描，再次展开磁盘子菜单查看结果。

MacBook 电池状态区分“电池供电”“正在充电”“已充满”和“外接电源 · 未充电”；系统未提供预计时间时不显示时间。无内置电池的 Mac 会明确显示“无电池”。菜单不包含“刷新小组件”和“查看小组件状态”入口，资源窗口也不显示手动刷新小组件按钮。
...
目录扫描包含扫描范围内的隐藏项目，不跟随符号链接、不跨卷，硬链接按同一文件去重并归入先扫描到的项目。隐私目录跳过、权限不足、扫描取消或扫描中均会显示相应状态；结果可能不完整。APFS 克隆共享块和快照可能使扫描合计与实际可释放空间不同。
