...
菜单栏的 CPU、内存、磁盘、功率、电池复选框支持**一次展开连续多选**，选择一项不会自动关闭菜单，顶部状态栏立即显示新的组合。全部取消时仅显示图标。菜单展开期间，顶部状态栏和菜单中的采样数值均继续更新。选择结果与窗口同步并保存。
...
三种样式均支持任意指标组合、约每秒更新和新版本提示，自动适配深浅色。样式选择会保存，菜单与资源面板同步；在菜单中切换立即应用顶部布局，菜单保持展开。固定组合和样式下，读数预留宽度不随每秒采样变化；主动切换组合或样式时按新布局调整宽度。
