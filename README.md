...
当前版本：**3.8.2**。安装后的应用和小组件名称均为 **“系统状态”**。

## 下载安装

**[下载安装包：MacPulse Widget 3.8.2（M 系列 / Intel 通用）](https://github.com/qwer8856/MACPulse-widget/releases/download/v3.8.2/MACPulse-Widget-v3.8.2-macOS-universal.zip)**

[版本发布页与校验文件](https://github.com/qwer8856/MACPulse-widget/releases/tag/v3.8.2)
...
三种样式均支持任意指标组合、约每秒更新和新版本提示，自动适配深浅色。网络读数始终排在已选指标最后，并以淡竖线与前一项分隔；在全部三种样式中均为上方上传、下方下载的两行布局，数值保持对齐。网络单独显示时不显示前方分隔线。样式选择会保存，菜单与资源面板同步；在菜单中切换立即应用顶部布局，菜单保持展开。固定组合和样式下，读数预留宽度不随每秒采样变化；主动切换组合或样式时按新布局调整宽度。
