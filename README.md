...
原生 macOS 资源面板、可选菜单栏监控与桌面小组件。查看各核心 CPU、进程内存、磁盘容量、网络速率、电池与进程 CPU 能耗，并由用户选择退出进程。资源面板使用普通窗口，不会置顶。

当前版本：**3.8.1**。安装后的应用和小组件名称均为 **“系统状态”**。

## 下载安装

**[下载安装包：MacPulse Widget 3.8.1（M 系列 / Intel 通用）](https://github.com/qwer8856/MACPulse-widget/releases/download/v3.8.1/MACPulse-Widget-v3.8.1-macOS-universal.zip)**

[版本发布页与校验文件](https://github.com/qwer8856/MACPulse-widget/releases/tag/v3.8.1)
...
3. 在“总览与设置”中按需开启“在菜单栏显示”，分别勾选 CPU、内存、磁盘、功率、电池和网络，可任意组合；电池选项适用于有内置电池的 MacBook。
...
版本号、检查更新和下载按钮与“资源面板”、隐藏和退出按钮合并在同一操作行，不再占用底部独立区域。平时显示当前版本，有新版本时显示“当前版本 → 新版本”。悬停版本号可查看完整版本信息和检查结果，检查失败时版本号变为橙色。点击循环箭头可手动检查，点击下载图标打开对应的 Releases 页面，下载并替换应用。资源面板的更新区域保留完整信息。检查更新不会自动安装；失败不会弹窗或影响资源监控。
...
| 页面 | 内容 |
| --- | --- |
| 总览与设置 | 左侧集中显示 CPU、内存、磁盘、网络、功率与电池状态，右侧为菜单栏、自启设置和系统工具 |
| CPU | 总体利用率，默认按 CPU 占用从高到低排列的进程 |
| 内存 | 总内存比例、应用/固定/压缩内存、交换空间、内存压力，按内存用量降序的进程及占比 |
| 磁盘 | 总容量、已用容量、可用容量及已用比例进度条 |
| 网络 | 实时下载与上传速率、已连接 Wi-Fi/以太网、本次运行累计流量、最近 60 个样本趋势和手动测速 |
| 功率 | 供电功率、瓦数刻度，以及按 CPU 能耗估算降序排列的进程 |
| 电池 | 电量、设计与充满容量、充放电状态、距充满或预计续航、健康度、循环次数、温度和电池净功率 |

资源窗口不显示各核心 CPU 图表；核心数量与各核心占用保留在菜单栏的 CPU 子菜单中。总览各项读数在指标行内垂直居中。窗口采用紧凑布局，支持调整大小。
...
菜单栏的 CPU、内存、磁盘、功率、电池、网络复选框支持**一次展开连续多选**，选择一项不会自动关闭菜单，顶部状态栏立即显示新的组合。全部取消时仅显示图标。菜单展开期间，顶部状态栏和菜单中的采样数值均继续更新。选择结果与窗口同步并保存。

在菜单的“显示内容”下方，或资源面板的“总览与设置”中，使用“菜单栏样式”切换右上角的排列方式：

| 样式 | 排列方式 |
| --- | --- |
| 默认横排 | 保留名称与读数的宽松横向间距，升级后默认使用此样式 |
| 紧凑图标 | 每项使用对应图标与读数，减少文字占用 |
| 上下双行 | 上方为名称、下方为读数，电池上方显示电池及充电图标 |

三种样式均支持任意指标组合、约每秒更新和新版本提示，自动适配深浅色。网络读数始终排在已选指标最后，并以淡竖线与前一项分隔；在全部三种样式中均为上方下载、下方上传的两行布局，数值保持对齐。网络单独显示时不显示前方分隔线。样式选择会保存，菜单与资源面板同步；在菜单中切换立即应用顶部布局，菜单保持展开。固定组合和样式下，读数预留宽度不随每秒采样变化；主动切换组合或样式时按新布局调整宽度。

主菜单不再单独显示内存压力和交换空间，相关数据保留在内存详情与资源面板中。

有内置电池的 MacBook 勾选“电池”后，右上角显示电池图标和剩余百分比，可与其他指标任意组合。图标随电量变化，正在充电时显示闪电标记；开启 macOS 低电量模式时图标显示黄色，电量未知时显示 `--`。无内置电池时该选项置灰，顶部不显示电池读数。电池选项默认不勾选，升级保留已有选择。
...
**主菜单每个指标右侧都有原生展开箭头**。选择 CPU、内存、磁盘、网络、功率或电池，才展开对应子菜单；主菜单只保留概要，底部不再常驻显示 CPU 图表或电池详情。系统根据屏幕剩余空间决定子菜单展开方向。
...
| 网络 | 下载/上传速率、已连接网卡、本次接收/发送、最近 60 个速率样本和手动测速 |
...
- **资源面板与菜单栏约每秒采样一次**，菜单展开期间也会更新。网络页使用相同采样间隔显示实际下载/上传速率、Wi-Fi/以太网和本次累计流量；未采样前显示“等待网络采样”，首个有效样本的速率显示 `--`。仅展开菜单时，各核心与电池信息仍会持续采样；打开后的首个 CPU 采样间隔可能显示 `--`。
- **网络测速仅在用户点击“开始测速”后运行。** 它调用 macOS 的 `networkQuality`，约 15 秒，可取消，显示下载/上传 Mbps 和延迟 ms；可能消耗较多流量，结果受测试服务器、网络路径和代理影响，也不会自动启动。
- **真正的 WidgetKit 桌面小组件无法保证每秒刷新。** 应用内置了 1 秒后的刷新请求，但实际执行由 macOS 的刷新预算、节能策略和可见状态决定；没有公开接口能强制每秒采集并重绘。
- 桌面小组件不新增网络速率，仍只显示系统状态读数并按 macOS 的调度刷新。
...
| 网络速率 | 按系统 `SCNetworkInterface` 类型识别已连接的 Wi-Fi 或以太网，并读取其字节计数差值，按 B/s、KB/s 或 MB/s 自动换算；首样本、睡眠唤醒后和未识别网卡时不补造速率。物理网卡经过代理的流量会计入；VPN、回环和其他虚拟接口不叠加统计，以避免重复计算 |
| 网络测速 | 用户手动运行系统 `networkQuality` 的结果，下载/上传单位 Mbps，延迟单位 ms；结果会随测试服务器、网络路径和代理变化 |
...
<img src="docs/images/menu-styles-light.png" width="760" alt="默认横排、紧凑图标、上下双行三种菜单栏样式的浅色对比">
<img src="docs/images/menu-styles-dark.png" width="760" alt="三种菜单栏样式的深色对比，包含电池充电图标">
...
下图使用模拟速率和完成测速结果，展示网络详情页的布局；不代表当前网络或设备的实际测速值。

<img src="docs/images/network-details.png" width="700" alt="网络详情页，显示模拟的上下行速率、联网网卡、趋势和完成测速结果">
...
<img src="docs/images/menu-battery-option.png" width="380" alt="MacBook 电池图标与剩余百分比，显示内容支持任意组合">
